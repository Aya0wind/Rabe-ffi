...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_bsw_encrypt(public_key: *const c_void, policy: *const c_char, text: *const c_char, text_length: usize) -> *const c_void {
    let public_key = (public_key as *const CpAbePublicKey).as_ref();
...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_bsw_decrypt(cipher: *const c_void, secret_key: *const c_void) -> CBoxedBuffer {
...
mod test {
    ...
    use crate::cp_abe::bsw::{rabe_bsw_init, rabe_cp_bsw_ciphertext_to_json, rabe_cp_bsw_decrypt, rabe_cp_bsw_encrypt, rabe_cp_bsw_free_ciphertext, rabe_cp_bsw_free_master_key, rabe_cp_bsw_free_public_key, rabe_cp_bsw_generate_secret_key, rabe_cp_bsw_public_key_to_json, rabe_cp_bsw_secret_key_to_json};
    ...
    fn test() {
        unsafe {
            ...
            let secret_key = rabe_cp_bsw_generate_secret_key(public_key, master_key, attr_ptr.as_ptr(), attr.len());
            assert!(!secret_key.is_null());
            let policy = CString::new("\"a\" and \"b\"").unwrap();
            let text = CString::new("hello world").unwrap();
            let cipher = rabe_cp_bsw_encrypt(public_key,
                                             policy.as_ptr(),
                                             text.as_ptr(),
                                             "hello world".len());
