...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_aw11_generate_auth(global_key: *const c_void,
                                                    attrs: *const *const c_char,
                                                    attr_len: usize) -> Aw11AuthGenResult {
...
pub unsafe extern "C" fn rabe_cp_aw11_encrypt(
    ...
    if let Some(global_key) = global_key {
        ...
        let public_keys = std::slice::from_raw_parts(public_keys, public_keys_len).iter().map(|x| {
            (*x as *const Aw11PublicKey).read()
        }).collect::<Vec<_>>();
        let cipher = encrypt(
            global_key,
            &public_keys,
...
mod test {
    ...
    use crate::cp_abe::aw11::{rabe_aw11_init, rabe_cp_aw11_ciphertext_to_json, rabe_cp_aw11_decrypt, rabe_cp_aw11_encrypt, rabe_cp_aw11_free_ciphertext, rabe_cp_aw11_free_master_key, rabe_cp_aw11_free_public_key, rabe_cp_aw11_generate_auth, rabe_cp_aw11_generate_secret_key, rabe_cp_aw11_public_key_to_json, rabe_cp_aw11_secret_key_to_json};
    ...
    fn test() {
        unsafe {
            ...
            let cipher = rabe_cp_aw11_encrypt(global_key,
                                              public_keys.as_ptr(),
                                              public_keys.len(),
                                              policy.as_ptr(),
                                              text.as_ptr(),
