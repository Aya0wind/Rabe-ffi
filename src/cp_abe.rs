...
use crate::common::{DecryptResult};


#[no_mangle]
pub unsafe extern "C" fn rabe_deserialize_cp_sec_key(json: *const c_char) -> *const c_void{
...
#[no_mangle]
pub unsafe extern "C" fn rabe_deserialize_cp_cipher(json: *const c_char) -> *const c_void{
...
#[no_mangle]
pub unsafe extern "C" fn rabe_generate_cp_sec_key(master_key: *const c_void, attr: *const *const c_char, attr_len: usize) -> *const c_void {
...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_encrypt(pub_key: *const c_void, policy: *const c_char, text: *const c_char, text_length: usize) -> *const c_void {
...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_decrypt(cipher: *const c_void, sec_key: *const c_void) -> DecryptResult {
...
#[no_mangle]
pub unsafe extern "C" fn rabe_free_cp_sec_key(sec_key: *const c_void) {
    let _ = Box::<Ac17CpSecretKey>::from_raw(sec_key as *mut Ac17CpSecretKey);
}

#[no_mangle]
pub unsafe extern "C" fn rabe_free_cp_cipher(cipher: *const c_void) {
    let _ = Box::<Ac17CpCiphertext>::from_raw(cipher as *mut Ac17CpCiphertext);
}

#[no_mangle]
pub unsafe extern "C" fn rabe_cp_sec_key_to_json(sec_key: *const c_void) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn rabe_cp_cipher_to_json(cipher: *const c_void) -> *mut c_char {
    let cipher = (cipher as *const Ac17CpCiphertext).as_ref().unwrap();
...
mod test {
    ...
    use super::{rabe_cp_cipher_to_json, rabe_cp_decrypt, rabe_cp_encrypt, rabe_free_cp_cipher, rabe_generate_cp_sec_key};
    ...
    fn test() {
        unsafe {
            ...
            let attr_key = rabe_generate_cp_sec_key(sec_key, attr_ptr.as_ptr(), attr.len());
            assert!(!attr_key.is_null());
            let policy = CString::new("\"a\" and \"b\"").unwrap();
            let text = CString::new("hello world").unwrap();
            let cipher = rabe_cp_encrypt(pub_key,
                                         policy.as_ptr(),
                                         text.as_ptr(),
                                         "hello world".len());
            assert!(!cipher.is_null());
            let result = rabe_cp_decrypt(cipher, attr_key);
            ...
            println!("{}", std::ffi::CStr::from_ptr(json).to_str().unwrap());
            rabe_free_json(json);
            let json = rabe_cp_cipher_to_json(cipher);
            println!("{}", std::ffi::CStr::from_ptr(json).to_str().unwrap());
            rabe_free_json(json);
            rabe_free_cp_cipher(cipher);
